export const members = [
  {
    slug: "godwin-eziechina-chukwu",
    title: "Mazi",
    role: "Founder",
    name: "Godwin Eziechina Chukwu",
    contact: {
      email: "gchukwu833@gmail.com",
      phone: "",
    },
    picture: "/team/mr-godwin-eziechina-chukwu.jpeg",
    intro:
      "Godwin Eziechina Chukwu is a visionary entrepreneur, philanthropist, and community leader. He has spearheaded numerous initiatives that have positively impacted individuals and communities, demonstrating resilience, innovation, and a passion for social development.",
    biography:
      "Born in Nigeria, Godwin Eziechina Chukwu is a robust and forward-thinking entrepreneur whose career has been defined by leadership, innovation, and service. Over the years, he has successfully launched and managed initiatives that foster economic growth, empower communities, and create opportunities for the less privileged. His philanthropic efforts reflect a deep commitment to social responsibility, while his entrepreneurial ventures showcase his ability to transform ideas into sustainable impact. As a community leader, Godwin continues to inspire others through mentorship, advocacy, and strategic partnerships aimed at driving progress and inclusivity.",
    education: [
      {
        school: "",
        qualification: "",
        year: null,
      },
    ],
    workExperience: [
      "Founder and leader of multiple entrepreneurial initiatives.",
      "Philanthropist with a focus on community empowerment.",
      "Community leader advocating for social and economic development.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "benjamin-nwaforcha-nwakelu",
    title: "Dr",
    role: "Chairman",
    name: "Benjamin Nwaforcha Nwakelu",
    contact: {
      email: "benjamnnwakelu@gmail.com",
      phone: "+2348037741092",
    },
    picture: "/team/dr-benjamin-nwaforcha-nwakelu.png",
    intro:
      "Dr. Benjamin Nwaforcha Nwakelu, an honest compassionate General Medical Practitioner for 24 years with 16 years experience in Family Medicine using PCCM and EBM. A Lecturer in Clinical Pharmacology and Therapeutics with Ph.D in view and a PGDE holder. Competence in seeing all categories of patients and founded Family Medicine Department in FMC Abakaliki in 2006.",
    biography:
      "Born on January 10, 1962, in Orofia Abagana, Anambra State, Nigeria, Dr. Benjamin Nwaforcha Nwakelu is a seasoned General Medical Practitioner and educator. With over two decades of clinical experience and a strong foundation in Family Medicine, he has contributed significantly to healthcare development in Nigeria. He founded the Family Medicine Department at FMC Abakaliki and currently lectures in Pharmacology and Therapeutics at AEFUNAI. His academic journey includes an MBBS from Abia State University, a PGDE from NUT Kaduna, and an M.Sc in Pharmacology and Therapeutics from EBSU, with a Ph.D in view at UNN. He is passionate about accessible healthcare and aims to establish a charity for the less privileged.",
    education: [
      {
        school: "University of Nigeria Nsukka",
        qualification: "Ph.D in Pharmacology and Therapeutics",
        year: null,
      },
      {
        school: "Ebonyi State University (EBSU)",
        qualification: "M.Sc in Pharmacology and Therapeutics",
        year: null,
      },
      {
        school: "NUT Kaduna",
        qualification: "PGDE",
        year: null,
      },
      {
        school: "University College Hospital (UCH) Ibadan",
        qualification: "Supernumerary Residency in Family Medicine",
        year: null,
      },
      {
        school: "Abia State University Uturu",
        qualification: "MBBS",
        year: null,
      },
    ],
    workExperience: [
      "Medical Practitioner for 24 years.",
      "Chief Medical Officer, Associate Family Physician and Pharmacologist.",
      "Founder of Family Medicine Department at FMC Abakaliki, Ebonyi State, Nigeria.",
      "Lecturer in Pharmacology and Therapeutics at AEFUNAI, Ebonyi State.",
      "Currently on contract at Alex Ekwueme Federal University Teaching Hospital Abakaliki (AEFUTHA).",
      "Published articles in Ethno-Pharmacology.",
      "Currently pursuing a Ph.D in Pharmacology and Therapeutics.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "asogwa-damian-chinedu",
    title: "Dr",
    role: "Member Medical Team",
    name: "Asogwa Damian Chinedu",
    contact: {
      email: "asogwadamian@gmail.com",
      phone: "+2348034266840",
    },
    picture: "/team/dr-asogwa-damian-chinedu.png",
    intro:
      "Dr. Asogwa Damian Chinedu is a compassionate and efficient Consultant Obstetrician and Gynecologist with over two decades of experience in maternal and reproductive health. He is known for his dedication to family planning, psychological medicine, and clinical teaching, and currently serves at Alex Ekwueme Federal University Teaching Hospital Abakaliki.",
    biography:
      "Born on May 21, 1969, in Umudim-Orba, Udenu LGA, Enugu State, Nigeria, Dr. Asogwa Damian Chinedu is a seasoned medical professional with a rich background in obstetrics, gynecology, and psychiatric nursing. He has worked in various capacities across Enugu and Ebonyi States, including Cottage Hospital Obolloafor and Federal Neuropsychiatric Hospital Enugu. He is actively involved in voluntary obstetrics schemes and has partnered with organizations like USAID, Pathfinder International, and MCSP. His academic journey includes an MBBS from Ebonyi State University, FMCOG from AE-FUTHA, and a Registered Psychiatric Nurse certification. He is passionate about community mobilization, fertility control, and human development.",
    education: [
      {
        school: "Alex Ekwueme Federal University Teaching Hospital, Abakaliki",
        qualification: "FMCOG",
        year: "2019",
      },
      {
        school: "Ebonyi State University",
        qualification: "MBBS",
        year: "2003",
      },
      {
        school: "School of Psychiatric Nursing",
        qualification: "Registered Psychiatric Nurse",
        year: "1993",
      },
      {
        school: "Boys High School, Orba",
        qualification: "SSCE",
        year: "1988",
      },
      {
        school: "Central School, Orba",
        qualification: "FSLC",
        year: "1981",
      },
    ],
    workExperience: [
      "Consultant Obstetrician and Gynecologist (2020–present).",
      "Alex Ekwueme Federal University Teaching Hospital, Abakaliki, Ebonyi State (2010–present).",
      "Medical Officer at Cottage Hospital Obolloafor (MOH, Enugu).",
      "Registered Psychiatric Nurse at Federal Neuropsychiatric Hospital, Enugu (1993–1996).",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "duru-uchenna-stephen",
    title: "Dr",
    role: "Secretary",
    name: "Duru Uchenna Stephen",
    contact: {
      email: "accapellah2@gmail.com",
      phone: "+2348038067948",
    },
    picture: "/team/dr-duru-uchenna-stephen.jpeg",
    intro:
      "Dr. Duru Uchenna Stephen is an experienced Medical Officer with a strong background in hospital and healthcare practice. Known for his high sense of responsibility, excellent communication skills, and ability to manage diverse patient conditions, he is passionate about self-development and thrives in collaborative medical environments.",
    biography:
      "Born in Umuakah, Isu Njaba LGA of Imo State, Nigeria, Dr. Duru Uchenna Stephen is a dedicated medical professional with a proven track record in clinical care and public health. He has served in multiple medical institutions across Ebonyi and Imo States, including Ananda Merga Universal Relief Team and Sudan United Mission Hospital. His academic journey includes an MBBS from the University of Benin and several professional certifications in HIV management, emergency obstetrics, and surgical procedures. He is also proficient in IT and committed to continuous learning and service excellence.",
    education: [
      {
        school: "University of Benin, Edo State",
        qualification: "MBBS",
        year: "2014",
      },
      {
        school: "St Charles Special Science School, Onitsha, Anambra State",
        qualification: "NECO 'O' Level",
        year: "2002",
      },
      {
        school: "NYSC, Ebonyi State",
        qualification: "NYSC Discharged Certificate",
        year: "2018",
      },
    ],
    workExperience: [
      "Medical Officer at Ananda Merga Universal Relief Team, Abakaliki, Ebonyi State (2019–present).",
      "Medical Officer at Sudan United Mission Hospital, Iziogo, Ebonyi State (2017–2019).",
      "Medical Officer at Suncel Health Foundation, Owerri, Imo State (2016–2017).",
      "House Officer at Federal Medical Centre, Owerri, Imo State (2015–2016).",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "nwadike-jacinta-chizoba",
    title: "Dr",
    role: "Member Medical Team",
    name: "Nwadike Agbahia Jacinta Chizoba",
    contact: {
      email: "chizobajlo212@gmail.com",
      phone: "+2348064260184",
    },
    picture: "/team/dr-nwadike-jacinta-chizoba.png",
    intro:
      "Dr. Nwadike Jacinta Chizoba is a registered physiotherapist with over 15 years of clinical experience. She currently serves as Chief Physiotherapist at Alex Ekwueme Federal University Teaching Hospital Abakaliki, where she specializes in patient-centered rehabilitation and evidence-based physiotherapy care.",
    biography:
      "Born on September 17, 1985, in Isu LGA, Imo State, Nigeria, Dr. Jacinta Chizoba is a dedicated physiotherapist with a strong background in clinical practice and rehabilitation. She is registered with the Medical Rehabilitation Therapists Board of Nigeria and has worked across various hospital settings, including UNTH Enugu and Kwara State Specialist Hospital. Her expertise spans pelvic dysfunction, geriatric care, neurology, and musculoskeletal physiotherapy. She also runs her own private clinic, where she delivers high-quality, individualized care. Her work emphasizes evidence-based practice, virtual consultations, and service development in physiotherapy.",
    education: [
      {
        school: "University of Nigeria, Enugu Campus",
        qualification: "Physiotherapy Degree",
        year: "2010",
      },
      {
        school: "Kings College, Kano",
        qualification: "Secondary School Certificate",
        year: "2000",
      },
      {
        school: "Umuzike Primary School",
        qualification: "First School Leaving Certificate",
        year: "1995",
      },
    ],
    workExperience: [
      "Chief Physiotherapist at Alex Ekwueme Federal University Teaching Hospital, Ebonyi State.",
      "NYSC Physiotherapist at Kwara State Specialist Hospital, Sobi, Ilorin (2012–2013).",
      "Intern Physiotherapist at University Teaching Hospital, Ituku/Ozalla, Enugu (2010–2011).",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "odoh-ekenechukwu-gabriel",
    title: "Dr",
    role: "Member Medical Team",
    name: "Odoh Ekenechukwu Gabriel",
    contact: {
      email: "ekenechukwusabinus@gamil.com",
      phone: "+2348068441558",
    },
    picture: "/team/mr-odoh-ekenechukwu-gabriel.jpeg",
    intro:
      "Odoh Ekenechukwu Gabriel is a passionate paediatric physiotherapist with a creative approach to holistic rehabilitation. He currently serves at Alex Ekwueme Federal University Teaching Hospital Abakaliki, where he specializes in child-friendly therapy and evidence-based physiotherapy practices.",
    biography:
      "Born on December 12, 1977, in Ikem, Isi-Uzo LGA, Enugu State, Nigeria, Odoh Ekenechukwu Gabriel is a dedicated physiotherapist with a strong background in paediatric rehabilitation and clinical care. His career spans roles at St. Joseph Institute of Physiotherapy, Symbol Hospital Awkunan, and AE-FUTHA Abakaliki. He holds a degree from the University of Nigeria Nsukka and has additional training from IMT Enugu. Known for his leadership, counseling, and teaching skills, he is committed to improving patient outcomes through compassionate care and community engagement.",
    education: [
      {
        school: "University of Nigeria, Nsukka",
        qualification: "Physiotherapy Degree",
        year: "2007",
      },
      {
        school: "Institute of Management and Technology, Enugu",
        qualification: "Diploma",
        year: "2001",
      },
      {
        school: "Comprehensive Secondary School, Akpasha",
        qualification: "Secondary School Certificate",
        year: "1995",
      },
      {
        school: "Army Children School, Awkunan, Enugu",
        qualification: "Primary School Certificate",
        year: "1989",
      },
      {
        school: "Umuaram Central School, Ikem, Isi-Uzo LGA, Enugu State",
        qualification: "First School Leaving Certificate",
        year: null,
      },
    ],
    workExperience: [
      "Physiotherapist at Alex Ekwueme Federal University Teaching Hospital, Abakaliki.",
      "Physiotherapist at Symbol Hospital, Awkunan, Enugu (2010–2011).",
      "Physiotherapist at St. Joseph Institute of Physiotherapy, Enugu (2010).",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "obiorah-obianuju-ifeyinwa",
    title: "Dr",
    role: "Member Medical Team",
    name: "Obiorah Obianuju Ifeyinwa",
    contact: {
      email: "ujuobiorah3@gmail.com",
      phone: "+2348063631084",
    },
    picture: "/team/dr-obiorah-obianuju-ifeyinwa.png",
    intro:
      "Dr. Obiorah Obianuju Ifeyinwa is a dedicated physiotherapist with over a decade of experience in clinical practice and rehabilitation. She currently serves at Alex Ekwueme Federal University Teaching Hospital Abakaliki, Ebonyi State, and is known for her commitment to patient care and professional excellence.",
    biography:
      "Born in Enugu, Nigeria, and originally from Nawfia in Njikoka LGA of Anambra State, Dr. Obiorah Obianuju Ifeyinwa is a seasoned physiotherapist with a strong background in hospital-based care. Her career spans roles at AE-FUTHA Abakaliki, Usman Danfodiyo University Teaching Hospital Sokoto, and UNTH Enugu. She holds a degree from the University of Nigeria and has built a reputation for compassionate care and clinical expertise. Her journey reflects a deep commitment to health service delivery and continuous professional growth.",
    education: [
      {
        school: "University of Nigeria",
        qualification: "Physiotherapy Degree",
        year: "2007",
      },
      {
        school: "Queen's School, Enugu State",
        qualification: "Secondary School Certificate",
        year: "1998",
      },
      {
        school: "WTC Primary School, Enugu State",
        qualification: "First School Leaving Certificate",
        year: "1992",
      },
    ],
    workExperience: [
      "Physiotherapist at Alex Ekwueme Federal University Teaching Hospital, Abakaliki, Ebonyi State (2011–present).",
      "Physiotherapist at Usman Danfodiyo University Teaching Hospital, Sokoto State (2010–2011).",
      "Youth Corps Member at Usman Danfodiyo University Teaching Hospital, Sokoto State (2009–2010).",
      "Intern Physiotherapist at University of Nigeria Teaching Hospital, Ituku/Ozalla, Enugu State.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "onyeso-cordelia-ihunanyachi",
    title: "Nurse",
    role: "Member Medical Team",
    name: "Onyeso Cordelia Ihunanyachi",
    contact: {
      email: "ihuu4love2000@gmail.com",
      phone: "+2348126623336",
    },
    picture: "/team/nurse-onyeso-cordelia-ihunanyachi.jpeg",
    intro:
      "Cordelia Ihunanyachi Onyeso is a passionate Nurse Midwife dedicated to life-saving care and safe deliveries. She currently serves at Ananda Marga Universal Relief Team and has over a decade of experience in maternal health and community-based nursing.",
    biography:
      "Born in Ngor-Okpala LGA, Imo State, Nigeria, Cordelia Onyeso is a professional Nurse Midwife with a BNSC in Nursing from Ebonyi State University. Her training includes School of Nursing Umuahia and School of Midwifery Amachara. She has served in various healthcare settings including AMURT, Ezzaofu Ultra Modern Health Center, and Ndioruta Primary Health Center. Cordelia is known for her compassionate care, dedication to humanity, and commitment to delivering quality healthcare. Outside her profession, she enjoys reading and singing.",
    education: [
      {
        school: "Ebonyi State University",
        qualification: "BNSC in Nursing",
        year: "2019",
      },
      {
        school: "School of Nursing, Umuahia, Abia State",
        qualification: "Registered Nurse (RN)",
        year: "2013",
      },
      {
        school: "School of Midwifery, Amachara, Umuahia, Abia State",
        qualification: "Registered Midwife",
        year: "2009",
      },
      {
        school: "Oforola Community, Owerri West",
        qualification: "WAEC",
        year: "2001",
      },
      {
        school: "Development Primary School, Obiangwu",
        qualification: "First School Leaving Certificate",
        year: null,
      },
    ],
    workExperience: [
      "Nurse Midwife at Ananda Marga Universal Relief Team (AMURT), Ebonyi State (2020–present).",
      "Midwife at Ezzaofu Ultra Modern Health Center, Izzi LGA, Ebonyi State (2010–2016).",
      "Part-time Midwife at Ndioruta Primary Health Center, Izzi LGA, Ebonyi State (2017–2019).",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "mbanwite-stella-chidinma",
    title: "Nurse",
    role: "Member Medical Team",
    name: "Mbanwite Stella Chidinma",
    contact: {
      email: "stellachidnma2015@gmail.com",
      phone: "+2348038559278",
    },
    picture: "/team/mbanwite-stella-chidinma.png",
    intro:
      "Nurse Stella Chidinma Mbanwite is a dedicated midwife with years of experience in maternal care and clinical nursing. She currently serves at Alex Ekwueme Federal University Teaching Hospital Abakaliki, where she provides compassionate and professional care to patients across diverse settings.",
    biography:
      "Born in Aba, Abia State, Nigeria, and originally from Ihitte Uboma LGA in Imo State, Nurse Stella Chidinma Mbanwite is a committed healthcare professional with a strong background in nursing and midwifery. She is currently pursuing advanced training at the College of Nursing Science, Midwifery AEFUTH, Abakaliki. Her career includes service at AE-FUTHA and participation in the National Youth Service Corps. She is passionate about maternal health, patient recovery, and evidence-based nursing practice.",
    education: [
      {
        school: "University of Benin",
        qualification: "Nursing Degree",
        year: "2009",
      },
      {
        school: "College of Nursing Science, Midwifery AEFUTH, Abakaliki",
        qualification: "Midwifery Training",
        year: "2025",
      },
      {
        school: "Ohabiam Secondary School, Aba, Abia State",
        qualification: "Secondary School Certificate",
        year: "1997",
      },
      {
        school: "Eziobu Primary School 1, Aba, Abia State",
        qualification: "First School Leaving Certificate",
        year: null,
      },
    ],
    workExperience: [
      "Nurse Midwife at Alex Ekwueme Federal University Teaching Hospital, Ebonyi State.",
      "NYSC Nursing Service.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "austin-abu-jane-uchenna",
    title: "Medical Laboratory Scientist",
    role: "Member Medical Team",
    name: "Austin-Abu Jane Uchenna",
    contact: {
      email: "jannyabu@aefutha.gov.ng",
      phone: "+2348035493644",
    },
    picture: "/team/mrs-austin-abu-jane-uchenna.png",
    intro:
      "Mrs. Austin-Abu Jane Uchenna is an ISO 15189:2022 certified Medical Laboratory Scientist with 25 years of experience. She is dedicated to applying medical laboratory standards to deliver accurate, reliable, and quality reports that enhance patient care and safety.",
    biography:
      "With over two decades of professional expertise, Mrs. Austin-Abu Jane Uchenna has established herself as a leader in medical laboratory science. She currently serves as the Director of Medical Laboratory Services and Head of Department at Alex Ekwueme Federal University Teaching Hospital Abakaliki (AEFUTHA), Ebonyi State, Nigeria. Her career is marked by contributions to policy formulation, quality assurance, and patient safety. She is pursuing a Ph.D. at the University of Nigeria, building on her PGD in Medical Laboratory Science and MSc in Haematology. A passionate mentor and leader, she has held key positions in professional associations, unions, and community organizations, earning multiple awards of excellence in leadership. Her interests lie in mentorship, quality leadership, and advancing laboratory medicine standards.",
    education: [
      {
        school: "University of Nigeria",
        qualification: "Ph.D in Medical Laboratory Science (in progress)",
        year: null,
      },
      {
        school: "University of Nigeria",
        qualification: "PGD in Medical Laboratory Science",
        year: "2004",
      },
      {
        school: "University of Nigeria",
        qualification: "M.Sc in Haematology",
        year: "2014",
      },
      {
        school: "St. Cyprian's Special Science School, Nsukka",
        qualification: "Secondary School Certificate (SSCE)",
        year: null,
      },
      {
        school: "O'Connor Street Primary School, Asata Enugu",
        qualification: "First School Leaving Certificate (FSLC)",
        year: "1983",
      },
    ],
    workExperience: [
      "Director of Medical Laboratory Services, Alex Ekwueme Federal University Teaching Hospital Abakaliki (AEFUTHA).",
      "Head of Department, Medical Laboratory Services, AEFUTHA.",
      "Policy formulation and implementation to ensure patient safety and satisfaction.",
      "Leadership roles in professional associations, unions, and community organizations.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "obi-chisom-winnifred",
    title: "Medical Laboratory Scientist",
    role: "Member Medical Team",
    name: "Obi Chisom Winnifred",
    contact: {
      email: "chisomobi561@gmail.com",
      phone: "+2348127503442",
    },
    picture: "/team/obi-chisom-winnifred.png",
    intro:
      "Obi Chisom Winnifred is a passionate medical laboratory scientist dedicated to advancing healthcare through diagnostic excellence. She currently serves at Alex Ekwueme Federal University Teaching Hospital, where she contributes to disease detection and patient care through laboratory analysis.",
    biography:
      "Born on August 27, 1999, in Anambra State, Nigeria, Obi Chisom Winnifred is a committed medical laboratory scientist with a strong foundation in clinical diagnostics and laboratory research. Her academic journey began at Faith Academy in Asaba and continued at the University of Calabar, where she earned her degree in Medical Laboratory Science. She currently works at AE-FUTHA, applying her skills in sample analysis, testing, and result interpretation to support patient treatment and disease management. Her future aspirations include contributing to scientific innovation and improving healthcare outcomes across Nigeria.",
    education: [
      {
        school: "University of Calabar",
        qualification: "Medical Laboratory Science Degree",
        year: "2023",
      },
      {
        school: "Faith Academy, Asaba, Delta State",
        qualification: "Secondary School Certificate",
        year: "2015",
      },
    ],
    workExperience: [
      "Medical Laboratory Scientist at Alex Ekwueme Federal University Teaching Hospital, Abakaliki.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "nwadike-gerald-chimezie",
    title: "Mr",
    role: "Medical Administration and Financial Officer",
    name: "Nwadike Gerald Chimezie",
    contact: {
      email: "nwadikegerald@gmail.com",
      phone: "+2348063289585",
    },
    picture: "/team/dr-nwadike-gerald-chimezie.jpg",
    intro:
      "Dr. Nwadike Gerald Chimezie is a civil servant and econometrician with extensive experience in research, planning, and policy development. He currently serves as a Research, Planning, and Policy Officer at SMEDAN, Abuja, and is deeply involved in entrepreneurship development, government partnerships, and empowerment programs across Nigeria.",
    biography:
      "Born on June 26, 1983, in Isu LGA, Imo State, Nigeria, Dr. Nwadike Gerald Chimezie is a seasoned econometrician and public servant. He has contributed significantly to the development of entrepreneurship modules, coordination of empowerment programs, and partnerships with government and international organizations. His career spans roles in research, data analysis, and managerial consultancy. He is actively involved in community service and academic peer review, and has received multiple awards for his strategic leadership and research contributions.",
    education: [
      {
        school: "Imo State University, Owerri",
        qualification: "Ph.D in Economics",
        year: "2023",
      },
      {
        school: "Ebonyi State University, Abakaliki",
        qualification: "M.Sc in Economics",
        year: "2014",
      },
      {
        school: "Ebonyi State University, Abakaliki",
        qualification: "B.Sc in Economics",
        year: "2008",
      },
      {
        school: "AppmartGroup International Ltd",
        qualification: "Computer Training Diploma",
        year: "2010",
      },
      {
        school: "Comprehensive Secondary School, Isunjaba, Imo State",
        qualification: "Secondary School Certificate (NECO 'O' Level)",
        year: "2002",
      },
      {
        school: "Community Primary School, Isunjaba, Imo State",
        qualification: "First School Leaving Certificate",
        year: "1995",
      },
    ],
    workExperience: [
      "Commercial Officer (Small and Medium Enterprises Development Agency of Nigeria (SMEDAN), Abuja).",
      "Station Project Manager (AppmartGroup International Ltd, 2018).",
      "Data Operator (Techvibes International Ltd, 2013).",
      "Research Assistant (Center for Economic Development Research and Intervention (CEDRI), Abakaliki, Ebonyi, Nigeria, 2014).",
      "Publication Research 26 in number.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "robert-ifeanacho-mbah",
    title: "Chief",
    role: "Head of Elders Counsel, Trustees Member",
    name: "Robert Ifeanacho Mbah",
    contact: {
      email: "robertmbah4@gmail.com",
      phone: "+2348033002156",
    },
    picture: "/team/chief-robert-ifeanacho-mbah.jpeg",
    intro:
      "Chief Robert Ifeanacho Mbah (Sir Robby) is a distinguished Estate Surveyor and Valuer, Knight of the Catholic Church, and respected community leader. He brings decades of professional expertise and spiritual guidance to his role as Head of Elders Counsel and Trustees Member of Obi Umunna Na Umuada United.",
    biography:
      "Chief Robert Ifeanacho Mbah, fondly known as Sir Robby, is a graduate of Estate Management and an Associate Member of the Nigerian Institution of Estate Surveyors & Valuers. He is a Registered Estate Surveyor and Valuer (ESV) and a Knight of the Catholic Church. As founder and consultant at R.I. Mbah & Co Estate Surveyors and Valuers, he has led projects in feasibility and viability appraisal, project risk analysis, property management, and development. His firm has worked extensively with Federal and State Governments on valuation for rating, sales, compensation, acquisition, oil spillage, mortgage, mergers, and capital value opinions. He was a pioneer member of the Lagos State Rent Tribunal, serving as Vice Chairman alongside magistrates, landlords, and tenant representatives. Beyond his professional achievements, Chief Mbah is deeply committed to mentoring youths, counseling them on life and excellence in their chosen fields. Married to a distinguished academic, he is blessed with five children and grandchildren. His leadership in Obi Umunna Na Umuada United reflects his dedication to community service, tradition, and faith.",
    education: [
      {
        school: "University (Estate Management program)",
        qualification: "Graduate in Estate Management",
        year: null,
      },
      {
        school: "Nigerian Institution of Estate Surveyors & Valuers",
        qualification: "Associate Membership",
        year: null,
      },
    ],
    workExperience: [
      "Consultant Estate Surveyor & Valuer, R.I. Mbah & Co.",
      "Feasibility & Viability Appraisal and Project Risk Analyst.",
      "Property Manager and Developer with diverse government experience.",
      "Pioneer member and Vice Chairman, Lagos State Rent Tribunal.",
      "Head of Elders Counsel and Trustees Member, Obi Umunna Na Umuada United.",
      "Knight of the Catholic Church, actively involved in faith and community leadership.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "joseph-ndubuisi-paul",
    title: "Mr",
    role: "Trustee Member, Obi Umunna Na Umuada United",
    name: "Joseph Ndubuisi Paul",
    contact: {
      email: "Ndubuisipaul94@gmail.com",
      phone: "+13234557322",
    },
    picture: "/team/mr-joseph-ndubuisi-paul.jpeg",
    intro:
      "Joseph Ndubuisi Paul is a multi-talented sportsman, coach, music artist, entrepreneur, farmer, and philanthropist. With an impressive background in athletics and community service, he has represented Nigeria internationally and continues to inspire through his diverse contributions.",
    biography:
      "Joseph Ndubuisi Paul is a dynamic individual whose career spans sports, music, entrepreneurship, farming, and philanthropy. He began his education at St. Augustine Catholic Primary School, Odo-Egbo, Ijebu-Ode, Ogun State, and later attended Adeola Odutola College, Ijebu-Ode, for his secondary studies. As a gifted athlete, he represented Nigeria in international competitions across weightlifting, races, javelin, and discus, earning multiple medals for the nation. Beyond athletics, Joseph is the Chief Executive Officer (CEO) of Runcity LLC, demonstrating his entrepreneurial vision and leadership. His creative side shines through his work as a music artist, with numerous songs released to audiences. Deeply committed to community development, he engages in farming and philanthropy, while mentoring young athletes as a coach. As a trustee member of Obi Umunna Na Umuada United, he contributes to the organization’s mission and growth. Through his achievements in sports, business, music, and service, Joseph Ndubuisi Paul continues to make a lasting impact on society.",
    education: [
      {
        school:
          "St. Augustine Catholic Primary School, Odo-Egbo, Ijebu-Ode, Ogun State",
        qualification: "First School Leaving Certificate",
        year: null,
      },
      {
        school: "Adeola Odutola College, Ijebu-Ode, Ogun State",
        qualification: "Secondary School Certificate",
        year: null,
      },
    ],
    workExperience: [
      "Represented Nigeria in international competitions (weightlifting, races, javelin, discus), earning multiple medals.",
      "Chief Executive Officer (CEO) of Runcity LLC.",
      "Music artist with numerous songs released.",
      "Sportsman and coach, mentoring young athletes.",
      "Farmer and philanthropist engaged in community development.",
      "Trustee Member of Obi Umunna Na Umuada United.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "duru-festus-uchechukwu",
    title: "Hon",
    role: "Director of Human Resources and Monitoring Directorate, Trustees Member",
    name: "Duru Festus Uchechukwu Fcgm",
    contact: {
      email: "durundu@gmail.com",
      phone: "",
    },
    picture: "/team/mr-duru-festus-uchechukwu.jpeg",
    intro:
      "Duru Festus Uchechukwu is a prolific writer, philanthropist, entrepreneur, and Fellow of Corporate Governance and Management. He is widely recognized for his leadership, authorship, and dedication to community development.",
    biography:
      "Mr. Duru Festus Uchechukwu Fcgm is a multifaceted professional whose career spans farming, writing, philanthropy, entrepreneurship, and management consultancy. He is the author of 'Causes of Business Failure in Nigeria,' which earned him the 2012 Good Authorship Award. A Fellow of Corporate Governance and Management, Festus has consistently demonstrated excellence in leadership and innovation. During his National Youth Service Corps (NYSC), he won the 'Best Corps Member Award' for initiating and establishing a Free Newspapers and Magazines Reading Center in Ikot Edor, Onna LGA, Akwa Ibom State, supported by ExxonMobil Producing Nigeria Unlimited. He has served as a lecturer and is the Secretary General of Eunice Divine Care Foundation, an NGO dedicated to supporting the girl child, widows, and indigent persons. Festus has organized and participated in numerous seminars, workshops, and youth empowerment programs. His professional journey includes work with the Central Bank of Nigeria (CBN) and the African Leadership Forum (ALF) Entrepreneurship Development Center (EDC). He is also associated with Silent Properties Limited. In 2023, he contested as a candidate for Nigeria’s Federal House of Representatives. Currently, he serves as Director of Human Resources and Monitoring Directorate and Trustee Member of Obi Umunna Na Umuada United, continuing his commitment to leadership and service.",
    education: [
      {
        school: "Corporate Governance and Management Institute",
        qualification: "Fellow of Corporate Governance and Management (Fcgm)",
        year: null,
      },
    ],
    workExperience: [
      "Farmer, prolific writer, philanthropist, author, and entrepreneur.",
      "Winner of the 2012 Good Authorship Award for 'Causes of Business Failure in Nigeria'.",
      "Federal House of Representatives Candidate, Nigeria (2023).",
      "Best Corps Member Award during NYSC for initiating a Free Newspapers and Magazines Reading Center.",
      "Lecturer and Secretary General of Eunice Divine Care Foundation.",
      "Organizer and participant in seminars, workshops, trainings, and youth empowerment programs.",
      "Management Consultant with experience at CBN and ALF Entrepreneurship Development Center.",
      "Associated with Silent Properties Limited.",
      "Director of Human Resources and Monitoring Directorate, Obi Umunna Na Umuada United.",
      "Trustee Member of Obi Umunna Na Umuada United.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "lilianbosco-chinonye-opara",
    title: "Sr",
    role: "Treasurer, Trustees Member",
    name: "LilianBosco Chinonye Opara",
    contact: {
      email: "lilianbosco1@gmail.com",
      phone: "",
    },
    picture: "/team/sr-lilianbosco-chinonye-opara.jpeg",
    intro:
      "Sr. LilianBosco Chinonye Opara is a dedicated member of the Missionary Sisters of the Blessed Sacrament (MSBS), serving as Treasurer and Trustee of Obi Umunna Na Umuada United. She combines her spiritual vocation with leadership and mentorship roles in education and community service.",
    biography:
      "Born and raised in Lagos, Sr. LilianBosco Chinonye Opara hails from Aboh Mbaise in Imo State, Nigeria. A committed member of the Missionary Sisters of the Blessed Sacrament (MSBS), she has devoted her life to faith, service, and leadership. She currently serves as Treasurer and Trustee of Obi Umunna Na Umuada United, contributing to the organization’s financial stewardship and mission. Professionally, she has held roles as General Secretary of the Missionary Sisters of the Blessed Sacrament and as a School Counsellor at Mater Misericordiae School, where she guided students in personal and academic development. With a Ph.D. in view, Sr. LilianBosco continues to pursue academic excellence while remaining deeply engaged in mentorship, community service, and spiritual leadership.",
    education: [
      {
        school: "University (specific institution not provided)",
        qualification: "Ph.D in progress",
        year: null,
      },
    ],
    workExperience: [
      "General Secretary, Missionary Sisters of the Blessed Sacrament (MSBS).",
      "School Counsellor, Mater Misericordiae School.",
      "Treasurer and Trustee Member, Obi Umunna Na Umuada United.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },

  {
    slug: "glory-ndukwe",
    title: "Ms",
    role: "Trustees Member",
    name: "Glory Ndukwe",
    contact: {
      email: "gloryndukwe446@gmail.com",
      phone: "",
    },
    picture: "/team/ms-glory-ndukwe.jpeg",
    intro:
      "Glory Ndukwe is an entrepreneur and business leader based in northern Italy. She serves as a Trustee Member of Obi Umunna Na Umuada United, contributing to the organization’s mission and growth.",
    biography:
      "Glory Ndukwe is a dedicated entrepreneur whose business activities are rooted in northern Italy. While she maintains privacy regarding her personal biography and educational background, her professional and community contributions speak volumes. As a Trustee Member of Obi Umunna Na Umuada United, she plays a vital role in supporting the organization’s vision and initiatives. Her entrepreneurial drive and international presence reflect her commitment to both business excellence and community development.",
    education: [
      {
        school: "",
        qualification: "Private",
        year: null,
      },
    ],
    workExperience: [
      "Entrepreneur and business leader based in northern Italy.",
      "Trustee Member of Obi Umunna Na Umuada United.",
    ],
    social: {
      linkedin: "",
      twitter: "",
      facebook: "",
    },
  },
];
